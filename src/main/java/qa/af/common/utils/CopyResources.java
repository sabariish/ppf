...
public final class CopyResources {
    ...
    public Optional<JarFile> jar(final Class<?> clazz) throws IOException {
        ...
        if (url != null) {
            ...
            if (jar.startsWith(CopyResources.JAR_URI_PREFIX) && bang != -1) {
                optional = Optional.of(
                    new JarFile(
                        jar.substring(CopyResources.JAR_URI_PREFIX.length(), bang)
                    )
                );
            }
        }
    ...
    private void writeToFile(final InputStream input, final File target)
        ...
        final byte[] buffer = new byte[CopyResources.BUFFER_SIZE];
